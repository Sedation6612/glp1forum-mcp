...
- **`get_thread_images`** — opt-in: download a thread's image attachments as image blocks so vision can read image-only pricing tables / stock boards. `max` caps the count (default 6).
